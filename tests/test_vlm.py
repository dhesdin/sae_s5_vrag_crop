...
def test_create_visual_element():
    ...
    visual_element = VisualElement(label="Test Label", description="Test Description", attributes={"key1": "value1", "key2": "value2"}, bbox=bbox)
...
def test_visual_element_default_bbox():
    visual_element = VisualElement(label="Test Label", description="Test Description")
...
def test_bouding_box_missing_fields():
    with pytest.raises(ValueError):
        BoundingBox(x_min=10, y_min=20, x_max=30)


def test_visual_element_missing_label():
    with pytest.raises(ValueError):
        VisualElement(description="Test Description")


def test_visual_element_missing_description():
    with pytest.raises(ValueError):
        VisualElement(label="Test Label")
